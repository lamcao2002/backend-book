...
router.post('/register', async function (req, res, next) {
  try {
    ...
    if (user === null) {
      ...
      return res.json({ code: 200, message: MESSAGES.REGISTER_SUCCESS, data: { userCreate } });
    } else {
      return res.json({ code: 400, errorMess: MESSAGES.USERNAME_EXISTED, data: null })
    }
  } catch (err) {
...
router.post('/login', async function (req, res, next) {
  try {
    const user = await UserModel.findOne({ username: req.body.username });
    if (user === null) { return res.json({ code: 400, errorMess: MESSAGES.USERNAME_WRONG, data: null }) };
    ...
    if (result) {
      ...
      return res.json({ code : 400, errorMess : MESSAGES.PASSWORD_WRONG, data : null });
    }
  } catch (err) {
    return res.json({ code: 400, errorMess: err.message, data: null });
  }
})
...
router.post('/changePassword', async function (req, res) {
  try {
    ...
    if (!user) {
      return res.json({ code: 400, errorMess: MESSAGES.USERNAME_WRONG, data: null });
    }
